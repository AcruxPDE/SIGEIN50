...
-- Description: Elimina un C_COLONIA
-- =============================================
CREATE PROCEDURE [ADM].[SPE_ELIMINA_C_COLONIA]
...
BEGIN
	...
	BEGIN TRY		   			
		...
		BEGIN
			...
			SET @V_EXIST_TRAN = 1
		END	
		--SE BORRA EL REGISTRO EN LA TABLA  ADM.C_COLONIA
		DELETE FROM ADM.C_COLONIA
		WHERE [ID_COLONIA] = @PIN_ID_COLONIA

		   SET @XML_RESULTADO = DBO.F_ERROR_CREAR_ENCABEZADO( @@ROWCOUNT, 1, 'SUCCESSFUL')
		SET @XML_RESULTADO = DBO.F_ERROR_INSERTAR_MENSAJES(@XML_RESULTADO, 'Se eliminó la colonia satisfactoriamente', 'ES')
		SET @XML_RESULTADO = DBO.F_ERROR_INSERTAR_MENSAJES(@XML_RESULTADO, 'Se eliminó la colonia satisfactoriamente', 'EN')
