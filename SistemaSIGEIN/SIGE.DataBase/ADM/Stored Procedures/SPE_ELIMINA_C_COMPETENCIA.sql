...
-- Description: Elimina un C_COMPETENCIA
-- =============================================
CREATE PROCEDURE [ADM].[SPE_ELIMINA_C_COMPETENCIA]
...
BEGIN
	...
	BEGIN TRY		   			
		...
		BEGIN
			...
			SET @V_EXIST_TRAN = 1
		END	
		--SE BORRA EL REGISTRO EN LA TABLA  ADM.C_COMPETENCIA
		DELETE FROM ADM.C_COMPETENCIA_NIVEL
		WHERE [ID_COMPETENCIA] = @PIN_ID_COMPETENCIA
		
		--SE BORRA EL REGISTRO EN LA TABLA  ADM.C_COMPETENCIA
		DELETE FROM ADM.C_COMPETENCIA
		WHERE [ID_COMPETENCIA] = @PIN_ID_COMPETENCIA
				
		SET @XML_RESULTADO = DBO.F_ERROR_CREAR_ENCABEZADO( @@ROWCOUNT, 1, 'SUCCESSFUL')
		SET @XML_RESULTADO = DBO.F_ERROR_INSERTAR_MENSAJES(@XML_RESULTADO, 'Se eliminó la competencia satisfactoriamente', 'ES')
		SET @XML_RESULTADO = DBO.F_ERROR_INSERTAR_MENSAJES(@XML_RESULTADO, 'Se eliminó la competencia satisfactoriamente', 'EN')
