...
-- Description: Inserta un nuevo registro en la tabla C_EMPRESA
-- =============================================
CREATE PROCEDURE [ADM].[SPE_INSERTA_ACTUALIZA_C_EMPRESA] 
		...
		, @PIN_CL_EMPRESA AS nvarchar(20)
		, @PIN_NB_EMPRESA AS nvarchar(100)
		, @PIN_NB_RAZON_SOCIAL AS nvarchar(200)
...
BEGIN  
	...
	,@CFE_SISTEMA DATETIME = dbo.F_GETDATE()
		...
		IF @PIN_TIPO_TRANSACCION='I'
	    	BEGIN
			--SE INSERTA EL REGISTRO EN LA TABLA  ADM.C_EMPRESA
			...
			VALUES
					   (@PIN_CL_EMPRESA
						, @PIN_NB_EMPRESA
						, @PIN_NB_EMPRESA--@PIN_NB_RAZON_SOCIAL
						,@CFE_SISTEMA
						, @PIN_CL_USUARIO_APP_CREA
						, @PIN_NB_PROGRAMA_CREA
					)			
		END ELSE BEGIN
			--SE ACTUALIZA EL REGISTRO EN LA TABLA  ADM.C_EMPRESA
			UPDATE ADM.C_EMPRESA SET
				[CL_EMPRESA] = @PIN_CL_EMPRESA
				, [NB_EMPRESA] = @PIN_NB_EMPRESA
				, [NB_RAZON_SOCIAL] = @PIN_NB_RAZON_SOCIAL
	...
	BEGIN CATCH		
		...
		IF @ERROR_CLAVE = 547
		BEGIN
			SET @ERROR_MENSAJE = 'Esta acción no se puede completar 
			porque la ' +@PIN_NB_EMPRESA + ' está actualmente asociado a un empleado'
		END
		ELSE
		BEGIN
