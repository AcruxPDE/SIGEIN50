...
-- Description: Inserta un nuevo registro en la tabla K_EVALUADO_PERIODO
-- =============================================
CREATE PROCEDURE [ADM].[SPE_INSERTA_ACTUALIZA_K_EVALUADO_PERIODO] 
	      @XML_RESULTADO XML = '' OUT       --APLICA PARA REGRESAR UN NÚMERO 0 PARA ERROR Y 1 PARA CORRECTO
    	, @PIN_ID_EVALAUDOR_PERIODO AS int
		, @PIN_ID_PERIODO AS int
		, @PIN_ID_EMPLEADO AS int
		, @PIN_ID_PUESTO AS int
		, @PIN_FG_PUESTO_ACTUAL AS int
		, @PIN_NO_CONSUMO_SUP AS int
		, @PIN_MN_CUOTA_BASE AS decimal(13,2)
		, @PIN_MN_CUOTA_CONSUMO AS decimal(13,2)
		, @PIN_MN_CUOTA_ADICIONAL AS decimal(13,2)
...
BEGIN  
	...
	,@CFE_SISTEMA DATETIME = dbo.F_GETDATE()
		...
		IF @PIN_TIPO_TRANSACCION='I'
	    	BEGIN
			--SE INSERTA EL REGISTRO EN LA TABLA  ADM.K_EVALUADO_PERIODO
			INSERT INTO ADM.K_EVALUADO_PERIODO
					   ([ID_PERIODO]
						, [ID_EMPLEADO]
						, [ID_PUESTO]
						, [FG_PUESTO_ACTUAL]
			...
			VALUES
					   (@PIN_ID_PERIODO
						, @PIN_ID_EMPLEADO
						, @PIN_ID_PUESTO
						, @PIN_FG_PUESTO_ACTUAL
						, @PIN_NO_CONSUMO_SUP
						, @PIN_MN_CUOTA_BASE
						, @PIN_MN_CUOTA_CONSUMO
						, @PIN_MN_CUOTA_ADICIONAL
						, @CFE_SISTEMA
						, @PIN_CL_USUARIO_APP_CREA
						, @PIN_NB_PROGRAMA_CREA
					)			
		END ELSE BEGIN
			--SE ACTUALIZA EL REGISTRO EN LA TABLA  ADM.K_EVALUADO_PERIODO
			UPDATE ADM.K_EVALUADO_PERIODO SET
				[ID_PERIODO] = @PIN_ID_PERIODO
				, [ID_EMPLEADO] = @PIN_ID_EMPLEADO
				, [ID_PUESTO] = @PIN_ID_PUESTO
				, [FG_PUESTO_ACTUAL] = @PIN_FG_PUESTO_ACTUAL
				, [NO_CONSUMO_SUP] = @PIN_NO_CONSUMO_SUP
				, [MN_CUOTA_BASE] = @PIN_MN_CUOTA_BASE
				, [MN_CUOTA_CONSUMO] = @PIN_MN_CUOTA_CONSUMO
				, [MN_CUOTA_ADICIONAL] = @PIN_MN_CUOTA_ADICIONAL
			...
			WHERE [ID_EVALUADOR_PERIODO] = @PIN_ID_EVALAUDOR_PERIODO
