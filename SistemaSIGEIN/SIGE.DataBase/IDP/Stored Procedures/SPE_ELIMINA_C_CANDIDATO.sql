...
-- Description: Elimina un registro de la tabla C_CANDIDATO
-- =============================================
CREATE PROCEDURE [IDP].[SPE_ELIMINA_C_CANDIDATO]
	@XML_RESULTADO XML OUT,      --APLICA PARA REGRESAR UN NÚMERO 0 PARA ERROR Y 1 PARA CORRECTO
	@POUT_ID_CANDIDATO INT,
...
BEGIN
	...
	BEGIN TRY		   			
		...
		BEGIN
			...
			SET @V_EXIST_TRAN = 1
		END	
		--SE BORRA EL REGISTRO EN LA TABLA  ADM.C_CANDIDATO
		DELETE FROM ADM.C_CANDIDATO
		WHERE ID_CANDIDATO = @POUT_ID_CANDIDATO

		SET @XML_RESULTADO = DBO.F_ERROR_CREAR_ENCABEZADO( @@ROWCOUNT, ERROR_NUMBER(), 'SUCCESSFUL')
	    SET @XML_RESULTADO = DBO.F_ERROR_INSERTAR_MENSAJES(@XML_RESULTADO, 'Se eliminó el candidato satisfactoriamente', 'ES')
		SET @XML_RESULTADO = DBO.F_ERROR_INSERTAR_MENSAJES(@XML_RESULTADO, 'Se eliminó el candidato satisfactoriamente', 'EN')
