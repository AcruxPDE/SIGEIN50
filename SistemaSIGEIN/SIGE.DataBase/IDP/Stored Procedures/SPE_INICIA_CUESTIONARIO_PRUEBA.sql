...
-- =============================================
CREATE PROCEDURE [IDP].[SPE_INICIA_CUESTIONARIO_PRUEBA] 
		...
		, @PIN_CL_TOKEN_EXTERNO AS UNIQUEIDENTIFIER
...
BEGIN  
	...
	DECLARE @V_EXIST_TRAN BIT = 0,
		...
		SELECT @V_ID_CUESTIONARIO = ID_CUESTIONARIO,
			   @V_CL_ESTADO = CL_ESTADO,
			   @V_NO_TIEMPO = NO_TIEMPO,
			   @V_FE_INICIO_ANT = FE_INICIO
		FROM IDP.K_PRUEBA
		WHERE ID_PRUEBA = @PIN_ID_PRUEBA
		--AND ID_PRUEBA_PLANTILLA = @PIN_ID_PRUEBA_PLANTILLA
		--AND CL_ESTADO = 'CREADA'
		AND (
				--(ID_CANDIDATO IS NULL AND ID_EMPLEADO =@PIN_ID_EMPLEADO)
				--	OR
				(/*ID_CANDIDATO = @PIN_ID_CANDIDATO AND*/ CL_TOKEN_EXTERNO = @PIN_CL_TOKEN_EXTERNO)
			);
		...
		ELSE IF(@V_CL_ESTADO = 'INICIADA') BEGIN
			...
			IF @V_NO_TIEMPO <= 0 BEGIN
				UPDATE IDP.K_PRUEBA
					SET CL_ESTADO = 'TERMINADA', FE_TERMINO = dbo.F_GETDATE()
				WHERE ID_PRUEBA = @PIN_ID_PRUEBA
			END
...
--EXECUTE [IDP].[SPE_INICIA_CUESTIONARIO_PRUEBA] 