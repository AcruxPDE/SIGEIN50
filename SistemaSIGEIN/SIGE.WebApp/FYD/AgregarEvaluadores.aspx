<%@ Page Title="" Language="C#" MasterPageFile="~/FYD/ContextFYD.master" AutoEventWireup="true" CodeBehind="AgregarEvaluadores.aspx.cs" Inherits="SIGE.WebApp.FYD.AgregarEvaluadores" %>
<asp:Content ID="Content1" ContentPlaceHolderID="headContexto" runat="server">
    ...
    <script type="text/javascript">
        ...
        function generateDataForParent() {
            ...
            var vListBoxEvaluado = $find("<%= rlbEvaluado.ClientID %>");
            vListBoxEvaluado.trackChanges();

            var itemsEvaluado = vListBoxEvaluado.get_items();

            for (var i = 0; i < itemsEvaluado.get_count() ; i++) {
                var item = itemsEvaluado.getItem(i);
                var itemValue = item.get_value();
                var itemText = item.get_text();

                var vEvaluado = {
                    idEvaluado: itemValue
                };

                vEvaluados.push(vEvaluado);
            }

            var vListBoxEvaluador = $find("<%= rlbEvaluador.ClientID %>");
            vListBoxEvaluador.trackChanges();

            var itemsEvaluador = vListBoxEvaluador.get_items();

            for (var i = 0; i < itemsEvaluador.get_count() ; i++) {
                var item = itemsEvaluador.getItem(i);
                var itemValue = item.get_value();
                var itemText = item.get_text();

                var vEvaluador = {
                    idEvaluador: itemValue
                };

                vEvaluadores.push(vEvaluador);
            }

            var vDatos = {
                clRolEvaluador: vClRolEvaluador,
                oEvaluado: vEvaluados,
                oEvaluador: vEvaluadores,
            }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderContexto" runat="server">
    ...
    <div class="divControlDerecha">
        <div class="ctrlBasico">
            <telerik:RadButton ID="btnGuardarConfiguracion" runat="server" Text="Aceptar" AutoPostBack="false" OnClientClicked="generateDataForParent"></telerik:RadButton>
        </div>
    </div>
