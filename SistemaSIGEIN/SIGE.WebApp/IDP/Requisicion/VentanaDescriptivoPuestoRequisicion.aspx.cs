...
{
    public partial class VentanaDescriptivoPuestoRequisicion : System.Web.UI.Page
    {
        #region Variables
        ...
        public List<E_EXPERIENCIA> vListaExperiencia
        {
        ...
        {
            get { return (int?)ViewState["vidRequisicion"]; }
            set { ViewState["vidRequisicion"] = value; }
        }

        private string vCrearAutorizar
        {
            get { return (string)ViewState["vCrearAutorizar"]; }
            set { ViewState["vCrearAutorizar"] = value; }
        }
        ...
        public void CargarDatos(int? pIdDescriptivo)
        {
            ...
            E_DESCRIPTIVO vDescriptivo = nDescriptivo.ObtieneDescriptivoRequisicion(pIdDescriptivo);
        ...
        public void GuardarPuesto(bool pFgCerrarVentana)
        {
            if (validarControles())
            {
                ...
                if (vXmlCA != null)
                {
                    ...
                    E_RESULTADO vResultado = neg.InsertaActualizaPuestoRequsicion(tran, vIdDescriptivo.Value, xml, xmlOcupacion, vidRequisicion, "", "N", "N", vClUsuario, vNbPrograma);
                    ...
                    if (vResultado.CL_TIPO_ERROR == E_TIPO_RESPUESTA_DB.SUCCESSFUL)
                    {
                        ...
                        var vpuesto = npuesto.ObtienePuestosRequisicion(pClPuesto: txtNombreCorto.Text).FirstOrDefault();
                        var id_puesto = 0;
                        var nbPuesto = "";
                        var estatus = "";
                        string vDsFuncionJavascript = "";

                        if (vpuesto != null)
                        {
                            id_puesto = vpuesto.ID_PUESTO;
                            nbPuesto = vpuesto.NB_PUESTO;
                            estatus = vpuesto.CL_ESTATUS;

                            vDsFuncionJavascript = "datosPuesto(" + id_puesto + ", '" + nbPuesto + "','" + estatus + "');";
                        }

                        //UtilMensajes.MensajeResultadoDB(rwmAlertas, vMensaje, vResultado.CL_TIPO_ERROR, pCallBackFunction: "");
                        ClientScript.RegisterStartupScript(GetType(), "script", vDsFuncionJavascript, true);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                if (Request.QueryString["idRequisicion"] != null)
                {
                    vidRequisicion = int.Parse(Request.QueryString["idRequisicion"]);

                }

                vCrearAutorizar = Request.QueryString["vCrearAutorizar"].ToString();
                ...
                //requisicion
                if (vCrearAutorizar == "Autorizar")
                {
                    pvwAutorizar.Visible = true;
        ...
        {
            DescriptivoNegocio nDescriptivo = new DescriptivoNegocio();
            E_DESCRIPTIVO vDescriptivo = nDescriptivo.ObtieneDescriptivoRequisicion(vIdDescriptivo);
        ...
        protected void btnAutorizar_Click(object sender, EventArgs e)
        {
            ...
            //var comentarios = radEditorAutorizar.Text;
            //var vNotificacion = nNotificacion.ObtieneRequisicion(pIdRequisicion: vidRequisicion).FirstOrDefault();

            //if (vNotificacion != null)
            //{
            //    vPuesto.ID_PUESTO = vNotificacion.ID_PUESTO;
            //}
            //else { vPuesto.ID_PUESTO = vIdDescriptivo; }
            //if (rcbAutorizar.Checked == false && radEditorAutorizar.Text == "")
            //{
            //    UtilMensajes.MensajeResultadoDB(rwmAlertas, "Ingresa el motivo del rechazo", E_TIPO_RESPUESTA_DB.ERROR, pCallBackFunction: "");

            //}
            //else
            //{
            //    if (rcbAutorizar.Checked == false)
            //    {
            //        autorizarechaza = "R";
            //    }
            //    else
            ...
            //    var correo = "";
            //    var nbCreaRequisicion = "";

            //    var vUsuarioInfo = nEmpleado.Obtener_M_EMPLEADO(ID_EMPLEADO: vNotificacion.ID_SOLICITANTE).FirstOrDefault();
            //    if (vUsuarioInfo != null)
            //    {
            //        nbCreaRequisicion = vUsuarioInfo.NB_EMPLEADO_COMPLETO;
            //        correo = vUsuarioInfo.CL_CORREO_ELECTRONICO;

            //    }
            //    if (correo != "")
            //    {
            //        if (vNotificacion.CL_ESTATUS == "EN REVISION")
            //        {
            ...
            //                vMensajeCorreo = vMensajeCorreo.Replace("[NB_PUESTO]", vNotificacion.NB_PUESTO);
            //                vMensajeCorreo = vMensajeCorreo.Replace("[DS_COMENTARIOS]", comentarios);
            //                vMensajeCorreo = vMensajeCorreo.Replace("[CL_REQUISICION]", vNotificacion.NO_REQUISICION);

            //                E_RESULTADO vResultado = neg.InsertaActualizaPuestoRequsicion("", vPuesto, vClUsuario, "Descriptivo", null, vidRequisicion, comentarios, autoriza, autorizarechaza);
            //                UtilMensajes.MensajeResultadoDB(rwmAlertas, "Envío procesado", E_TIPO_RESPUESTA_DB.SUCCESSFUL);
            //                string vMensaje = vResultado.MENSAJE.Where(w => w.CL_IDIOMA.Equals(vClIdioma.ToString())).FirstOrDefault().DS_MENSAJE;
            ...
            //            catch (Exception)
            //            {
            //                UtilMensajes.MensajeResultadoDB(rwmAlertas, "Envío no procesado", E_TIPO_RESPUESTA_DB.ERROR, pCallBackFunction: "");
            //            }

            //        }
            //        else
            //        {
            //            UtilMensajes.MensajeResultadoDB(rwmAlertas, "Este puesto ya está " + vNotificacion.CL_ESTATUS, E_TIPO_RESPUESTA_DB.WARNING, pCallBackFunction: "");

            //        }
            //    }
            //    else
            //    {
            //        UtilMensajes.MensajeResultadoDB(rwmAlertas, "No tienes correo asignado para enviar la notificación", E_TIPO_RESPUESTA_DB.WARNING, pCallBackFunction: "");
