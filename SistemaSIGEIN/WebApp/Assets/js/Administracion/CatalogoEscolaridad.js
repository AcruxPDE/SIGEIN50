...
app.controller('C_ESCOLARIDADController', function ($scope, $http, $modal) {
    ...
    $scope.titulo = C_CARRERA_TECNICA.TITULO_PANTALLA;
  
    //$scope.templateGrid_Escolaridad = function () {  
    //    return "<div class=\"toolbar\"> "+ 
 	//		"<div style=\"float:right\">  "+  
 	//			"<div style=\"float:left\">"+  
 	//				"<button id=\"btnExportarExcel_Grid_Escolaridad\" class=\"btn btn-link FontNegro\" onclick=\"ExportarExcel('Grid_Escolaridad')\"  > "+  
    ...
    //se carga el grid  
    $scope.CargarGrid_Escolaridad = function () {  
        NProgress.start();  //muestra el progreso en la pagina  

            $("#Grid_Escolaridad").kendoGrid({    
               ...
               , excel: {  
                   fileName: "Escolaridad.xlsx" //nombre del archivo a exportar   
              ...
              //  ,toolbar: kendo.template($scope.templateGrid_Escolaridad())   
                ,dataSource: {					   
                   ...
                   ,schema: {  
                       model: {  
                           fields: {  
                               NB_ESCOLARIDAD: { type: "string" } 
                               ,DS_ESCOLARIDAD: { type: "string" } 
               ...
               ,columns: [{ 
                   headerAttributes: {style: "text-align: center; "}  
                       , field:  "NB_ESCOLARIDAD"   
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: C_CARRERA_TECNICA.NB_ESCOLARIDAD // titulo de la columna 
                   ...
                   headerAttributes: {style: "text-align: center; "}  
                       , field:  "DS_ESCOLARIDAD"   
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: C_CARRERA_TECNICA.DS_ESCOLARIDAD // titulo de la columna 
                       , width:99 // ancho de la columna 
                       , hidden:false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: { 
                   ...
                   headerAttributes: {style: "text-align: center; "}  
                       ...
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: C_CARRERA_TECNICA.CL_NIVEL_ESCOLARIDAD // titulo de la columna 
                       , width:99 // ancho de la columna 
                       , hidden:false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: { 
                   ...
                   headerAttributes: {style: "text-align: center; "}  
                       ...
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: C_CARRERA_TECNICA.FG_ACTIVO // titulo de la columna 
                       , width:99 // ancho de la columna 
                       , hidden:false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: { 
               ...
               ,change: function (e) {  //evento que indica cuando cambia de elemento entre una y otra linea  
                   ...
                   $scope.entidad_Escolaridad = dataItem; //se asigna los datos a la variable de que se crea en el scope  
               }  
            });
    ...
    $scope.Add_Grid_Escolaridad = function () {  
    var Obj_Escolaridad = { //se crea la entidad para enviarla al sp 
        NB_ESCOLARIDAD: '' 
        ,DS_ESCOLARIDAD: '' 
    ...
    var modalInstance = $modal.open({ //se manda abrir la ventana modal para ingresar el registro 
        ...
        templateUrl: 'op_Escolaridad.html', //se le indica el html que se abrira 
        controller: 'Modal_Escolaridad', //se le indica el controlador del html 
        resolve: { //se le envia el objeto 
            OBJETO: function () { return Obj_Escolaridad; } 
        } 
    }); 
};  
  
    //inicia evento para edita el registro  
    $scope.Edit_Grid_Escolaridad = function () {  
    if ($scope.entidad_Escolaridad == undefined) { // se verifica que el registro este seleccionado  
        BootstrapDialog.show({ //se muestra el mensaje de error  
            title: C_CARRERA_TECNICA.TITULO_PANTALLA, //se trae el nombre de la pantalla  
    ...
    var datos = $scope.entidad_Escolaridad; //se asigna el registro a la variable de datos  
    var Obj_Escolaridad = { //se define el objeto que se enviara  
        //se le asigna valor a los campos de la entidad en base a lo que tiene el registro  
        NB_ESCOLARIDAD:  datos.NB_ESCOLARIDAD 
        ,DS_ESCOLARIDAD:  datos.DS_ESCOLARIDAD 
    ...
    $scope.Del_Grid_Escolaridad = function () {  
        if ($scope.entidad_Escolaridad == undefined) { //se verifica si la entidad que guarda el evento onchange del grid no se encuentra vacia  
            BootstrapDialog.show({ //en caso de que no se encuentre se mostrara un error  
                title: C_CARRERA_TECNICA.TITULO_PANTALLA, //se le pone el titulo especificado en la clase   
            ...
            buttons: [{ //se le agregan botones  
                ...
                action: function (result) { //se especifica la funcion que hara este boton  
                    if (result) { //en caso de que sea afirmativo  
                        $scope.Eliminar_Escolaridad();  //se ejecuta la funcion de elimina_?? especificada para borrar el registro  
                    }  
                }  
            }, {  
    ...
    $scope.Eliminar_Escolaridad = function () {  
    var datos = $scope.entidad_Escolaridad;  //se extrae el registro obtenido en el evento change del grid 
    $http({ //se especifica cual sera nuestro web service para ejecutarlo 
        url: "ws/OperacionesGral.svc/Delete_C_ESCOLARIDAD",  
        method: "POST",  
        data: { //se le pasan los parametros 
            ID_ESCOLARIDAD: datos.ID_ESCOLARIDAD, // se le pasa la llave primaria  
            usuario : TraerUsuario(), // se le pasa el usuario 
            //ojo modificar   
            programa : "CatalogoEscolaridad.html"  //se le pasa  
        },  
    ...
    .then(function (response) { // ejecucion de WS 
        if (response.data != true) {  //si el procedimiento regresa falso 
            BootstrapDialog.show({ //se mostrara un mensaje de error  
                title: C_ESCOLARIDAD.TITULO_PANTALLA,  //el titulo del mensaje esta en la clase Config que esta instanciada en appPrincipal.js en el atributo MENSAJEERROR  
            ...
            $("#Grid_Escolaridad").data("kendoGrid").dataSource.read(); //se manda a refrescar el grid  
            $("#modalDelete").modal('toggle');  
            BootstrapDialog.show({  //si el procedimiento regresa true 
                title: C_ESCOLARIDAD.TITULO_PANTALLA,  //el titulo del mensaje  
    ...
    function (response) {  //funcion que mostrara el mensaje si ocurre un error antes de ejecutar el sp  
        BootstrapDialog.show({  
            title: C_ESCOLARIDAD.TITULO_PANTALLA,  
    ...
    $scope.CargarGrid_Escolaridad ();  
});  
  
  
//MODAL QUE ABRE LA VENTANA DE INSERTAR/EDITAR C_ESCOLARIDAD 
app.controller('Modal_Escolaridad', function ($scope, $modalInstance, $http, OBJETO) {
    //habilita la animacion del popup para que se despliegue de arriba para abajo  
    $scope.animationsEnabled = true;
    ...
    $scope.customOptions = {
        ...
        dataTextField: "DS_NIVEL_ESCOLARIDAD", //texto a mostrar el combo cuando se seleccione
        dataValueField: "CL_NIVEL_ESCOLARIDAD", //valor del combo cuando se seleccione
        filter: "contains"
    };
    ...
    $scope.ID_ESCOLARIDAD = OBJETO.ID_ESCOLARIDAD;  
  
    //se generan las traducciones de las etiquetas, placeholder (ayuda en los componentes) 
    $scope.lbl_NB_ESCOLARIDAD = C_ESCOLARIDAD.NB_ESCOLARIDAD;  
    $scope.ph_NB_ESCOLARIDAD = C_ESCOLARIDAD.NB_ESCOLARIDAD_ph;  
    $scope.lbl_DS_ESCOLARIDAD = C_ESCOLARIDAD.DS_ESCOLARIDAD;  
    $scope.ph_DS_ESCOLARIDAD = C_ESCOLARIDAD.DS_ESCOLARIDAD_ph;  
    $scope.lbl_CL_NIVEL_ESCOLARIDAD = C_ESCOLARIDAD.CL_NIVEL_ESCOLARIDAD;  
    $scope.ph_CL_NIVEL_ESCOLARIDAD = C_ESCOLARIDAD.CL_NIVEL_ESCOLARIDAD_ph;  
    $scope.lbl_FG_ACTIVO = C_ESCOLARIDAD.FG_ACTIVO;  
    $scope.ph_FG_ACTIVO = C_ESCOLARIDAD.FG_ACTIVO_ph;  
  
    //se generan las traducciones Genericas 
    $scope.titulo = C_ESCOLARIDAD.TITULO_PANTALLA; 
    ...
    $scope.ok = function () { //en caso de que le den click en aceptar generara lo siguiente  
        ...
        $http({  
            ...
            data: {  
                ...
                //ojo modificar   
                programa: "InsertaEscolaridad.html",  
    ...
    $scope.cancel = function () {  
        $("#Grid_Escolaridad").data("kendoGrid").dataSource.read();  
