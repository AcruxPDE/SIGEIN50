app.controller('C_EVALUADOR_EXTERNOController', function ($scope, $http, $modal) { 
    ...
    $scope.titulo = C_EVALUADOR_EXTERNO.TITULO_PANTALLA;
    
    //SE CARGA EL TEMPLATE DEL TOOLBAR DEL GRID DONDE IRAN LOS BOTONES DE EDICION Y EXPORTACION  
    //$scope.templateGrid_EvaluadorExt = function () {  
    //    return "<div class=\"toolbar\"> "+ 
 	//		"<div style=\"float:right\">  "+  
 	//			"<div style=\"float:left\">"+  
 	//				"<button id=\"btnExportarExcel_Grid_EvaluadorExt\" class=\"btn btn-link FontNegro\" onclick=\"ExportarExcel('Grid_EvaluadorExt')\"  > "+  
    ...
    //se carga el grid  
    $scope.CargarGrid_EvaluadorExt = function () {  
        NProgress.start();  //muestra el progreso en la pagina  
        $(document).ready(function () {   
            $("#Grid_EvaluadorExt").kendoGrid({    
               ...
               , excel: {  
                   fileName: "EvaluadorExt.xlsx" //nombre del archivo a exportar   
                ...
                //,toolbar: kendo.template($scope.templateGrid_EvaluadorExt())   
                ,dataSource: {					   
                    ...
                    transport: {   
                        read: function (options) {    
                            $.ajax({    
                                type: "POST"    
                                ,url: "ws/OperacionesGral.svc/Get_C_EVALUADOR_EXTERNO"    
                                ,dataType: "json"    
                   ...
                   ,schema: {  
                       model: {  
                           fields: {  
                               CL_EVALUADOR_EXTERNO: { type: "string" } 
                               ,NB_EVALUADOR_EXTERNO: { type: "string" } 
                               ,DS_EVALUARDO_EXTERNO: { type: "string" } 
               ...
               ,columns: [{ 
                   headerAttributes: {style: "text-align: center; "}  
                       , field:  "CL_EVALUADOR_EXTERNO"   
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: C_EVALUADOR_EXTERNO.CL_EVALUADOR_EXTERNO // titulo de la columna 
                   ...
                   headerAttributes: {style: "text-align: center; "}  
                       , field:  "NB_EVALUADOR_EXTERNO"   
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: C_EVALUADOR_EXTERNO.NB_EVALUADOR_EXTERNO // titulo de la columna 
                       , width:99 // ancho de la columna 
                       , hidden:false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: { 
                   ...
                   headerAttributes: {style: "text-align: center; "}  
                       , field:  "DS_EVALUARDO_EXTERNO"   
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: C_EVALUADOR_EXTERNO.DS_EVALUARDO_EXTERNO // titulo de la columna 
                       , width:99 // ancho de la columna 
                       , hidden:false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: { 
                   ...
                   headerAttributes: {style: "text-align: center; "}  
                       ...
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: C_EVALUADOR_EXTERNO.FG_ACTIVO // titulo de la columna 
                       , width:99 // ancho de la columna 
                       , hidden:false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: { 
               ...
               ,change: function (e) {  //evento que indica cuando cambia de elemento entre una y otra linea  
                   ...
                   $scope.entidad_EvaluadorExt = dataItem; //se asigna los datos a la variable de que se crea en el scope  
               }  
        });  
    });  
...
$scope.Add_Grid_EvaluadorExt = function () {  
    var Obj_EvaluadorExt = { //se crea la entidad para enviarla al sp 
        CL_EVALUADOR_EXTERNO: '' 
        ,NB_EVALUADOR_EXTERNO: '' 
        ,DS_EVALUARDO_EXTERNO: '' 
        ,FG_ACTIVO: '' 
        ,ID_EVALUADOR_EXTERNO:  undefined  
    }; 
    var modalInstance = $modal.open({ //se manda abrir la ventana modal para ingresar el registro 
        ...
        templateUrl: 'op_EvaluadorExt.html', //se le indica el html que se abrira 
        controller: 'Modal_EvaluadorExt', //se le indica el controlador del html 
        resolve: { //se le envia el objeto 
            OBJETO: function () { return Obj_EvaluadorExt; } 
        } 
    }); 
};  
...
$scope.Edit_Grid_EvaluadorExt = function () {  
    if ($scope.entidad_EvaluadorExt == undefined) { // se verifica que el registro este seleccionado  
        BootstrapDialog.show({ //se muestra el mensaje de error  
            title: C_EVALUADOR_EXTERNO.TITULO_PANTALLA, //se trae el nombre de la pantalla  
    ...
    var datos = $scope.entidad_EvaluadorExt; //se asigna el registro a la variable de datos  
    var Obj_EvaluadorExt = { //se define el objeto que se enviara  
        //se le asigna valor a los campos de la entidad en base a lo que tiene el registro  
        CL_EVALUADOR_EXTERNO:  datos.CL_EVALUADOR_EXTERNO 
        ,NB_EVALUADOR_EXTERNO:  datos.NB_EVALUADOR_EXTERNO 
        ,DS_EVALUARDO_EXTERNO:  datos.DS_EVALUARDO_EXTERNO 
        ,FG_ACTIVO:  datos.FG_ACTIVO 
        ,ID_EVALUADOR_EXTERNO:  datos.ID_EVALUADOR_EXTERNO 
    };  
...
$scope.Del_Grid_EvaluadorExt = function () {  
    if ($scope.entidad_EvaluadorExt == undefined) { //se verifica si la entidad que guarda el evento onchange del grid no se encuentra vacia  
        BootstrapDialog.show({ //en caso de que no se encuentre se mostrara un error  
            title: C_EVALUADOR_EXTERNO.TITULO_PANTALLA, //se le pone el titulo especificado en la clase   
        ...
        buttons: [{ //se le agregan botones  
            ...
            action: function (result) { //se especifica la funcion que hara este boton  
                if (result) { //en caso de que sea afirmativo  
                    $scope.Eliminar_EvaluadorExt();  //se ejecuta la funcion de elimina_?? especificada para borrar el registro  
                }  
            }  
        }, {  
...
$scope.Eliminar_EvaluadorExt = function () {  
    var datos = $scope.entidad_EvaluadorExt;  //se extrae el registro obtenido en el evento change del grid 
    $http({ //se especifica cual sera nuestro web service para ejecutarlo 
        url: "ws/OperacionesGral.svc/Delete_C_EVALUADOR_EXTERNO",  
        method: "POST",  
        data: { //se le pasan los parametros 
            ID_EVALUADOR_EXTERNO: datos.ID_EVALUADOR_EXTERNO, // se le pasa la llave primaria  
            usuario : TraerUsuario(), // se le pasa el usuario 
            //ojo modificar   
            programa : "CatalogoEvaluadorExt.html"  //se le pasa  
        },  
    ...
    .then(function (response) { // ejecucion de WS 
        if (response.data != true) {  //si el procedimiento regresa falso 
            BootstrapDialog.show({ //se mostrara un mensaje de error  
                title: C_EVALUADOR_EXTERNO.TITULO_PANTALLA,  //el titulo del mensaje esta en la clase Config que esta instanciada en appPrincipal.js en el atributo MENSAJEERROR  
            ...
            $("#Grid_EvaluadorExt").data("kendoGrid").dataSource.read(); //se manda a refrescar el grid  
            $("#modalDelete").modal('toggle');  
            BootstrapDialog.show({  //si el procedimiento regresa true 
                title: C_EVALUADOR_EXTERNO.TITULO_PANTALLA,  //el titulo del mensaje  
    ...
    function (response) {  //funcion que mostrara el mensaje si ocurre un error antes de ejecutar el sp  
        BootstrapDialog.show({  
            title: C_EVALUADOR_EXTERNO.TITULO_PANTALLA,  
...
$scope.CargarGrid_EvaluadorExt ();  
});  
  
  
//MODAL QUE ABRE LA VENTANA DE INSERTAR/EDITAR C_EVALUADOR_EXTERNO 
app.controller('Modal_EvaluadorExt', function ($scope, $modalInstance, $http, OBJETO) {  
    $scope.CL_EVALUADOR_EXTERNO = OBJETO.CL_EVALUADOR_EXTERNO;  
    $scope.NB_EVALUADOR_EXTERNO = OBJETO.NB_EVALUADOR_EXTERNO;  
    $scope.DS_EVALUARDO_EXTERNO = OBJETO.DS_EVALUARDO_EXTERNO;  
    $scope.FG_ACTIVO = Boolean(OBJETO.FG_ACTIVO);  
    $scope.ID_EVALUADOR_EXTERNO = OBJETO.ID_EVALUADOR_EXTERNO;  
  
    //se generan las traducciones de las etiquetas, placeholder (ayuda en los componentes) 
    $scope.lbl_CL_EVALUADOR_EXTERNO = C_EVALUADOR_EXTERNO.CL_EVALUADOR_EXTERNO;  
    $scope.ph_CL_EVALUADOR_EXTERNO = C_EVALUADOR_EXTERNO.CL_EVALUADOR_EXTERNO_ph;  
    $scope.lbl_NB_EVALUADOR_EXTERNO = C_EVALUADOR_EXTERNO.NB_EVALUADOR_EXTERNO;  
    $scope.ph_NB_EVALUADOR_EXTERNO = C_EVALUADOR_EXTERNO.NB_EVALUADOR_EXTERNO_ph;  
    $scope.lbl_DS_EVALUARDO_EXTERNO = C_EVALUADOR_EXTERNO.DS_EVALUARDO_EXTERNO;  
    $scope.ph_DS_EVALUARDO_EXTERNO = C_EVALUADOR_EXTERNO.DS_EVALUARDO_EXTERNO_ph;  
    $scope.lbl_FG_ACTIVO = C_EVALUADOR_EXTERNO.FG_ACTIVO;  
    $scope.ph_FG_ACTIVO = C_EVALUADOR_EXTERNO.FG_ACTIVO_ph;  
    ...
    $scope.ok = function () { //en caso de que le den click en aceptar generara lo siguiente  
        ...
        if ($scope.ID_EVALUADOR_EXTERNO != undefined)  //se verifica que la clave primaria sea diferente de nulo  
            tipoOperacion = "A" //si se tiene la clave primaria se hara una modificacion en vez de insercion  
        var Obj_EvaluadorExt = {// se declara el objeto con la estructura del result para insertar o actualizar el registro 
            CL_EVALUADOR_EXTERNO: $scope.CL_EVALUADOR_EXTERNO 
 			,NB_EVALUADOR_EXTERNO: $scope.NB_EVALUADOR_EXTERNO 
 			,DS_EVALUARDO_EXTERNO: $scope.DS_EVALUARDO_EXTERNO 
 			, FG_ACTIVO: $scope.FG_ACTIVO,
 			ID_EVALUADOR_EXTERNO: $scope.ID_EVALUADOR_EXTERNO
        };  
        $http({  
            url: "ws/OperacionesGral.svc/Insert_update_C_EVALUADOR_EXTERNO",  
            method: "POST",  
            data: {  
                V_C_EVALUADOR_EXTERNO: Obj_EvaluadorExt,  
                usuario: TraerUsuario(),  
                //ojo modificar   
                programa: "InsertaEvaluadorExt.html",  
 		...
 		.then(function (response) {  
 		    if (response.data != true) {  
 		        ...
 		        if ($scope.ID_EVALUADOR_EXTERNO == undefined) {  
    ...
    $scope.cancel = function () {  
        $("#Grid_EvaluadorExt").data("kendoGrid").dataSource.read();  
