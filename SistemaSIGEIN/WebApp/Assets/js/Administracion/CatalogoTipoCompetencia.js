...
app.controller('S_TIPO_COMPETENCIAController', function ($scope, $http, $modal) {
    //habilita la animacion del popup para que se despliegue de arriba para abajo  
    $scope.animationsEnabled = true;

   

    $scope.titulo = S_TIPO_COMPETENCIA.TITULO_PANTALLA;
    ...
    //$scope.templateGrid_TipoCompetencia = function () {
    //    return "<div class=\"toolbar\"> " +
 	//		"<div style=\"float:right\">  " +
 	//			"<div style=\"float:left\">" +
 	//				"<button id=\"btnExportarExcel_Grid_TipoCompetencia\" class=\"btn btn-link FontNegro\" onclick=\"ExportarExcel('Grid_TipoCompetencia')\"  > " +
    ...
    //se carga el grid  
    $scope.CargarGrid_TipoCompetencia = function () {
        NProgress.start();  //muestra el progreso en la pagina  
        
        $(document).ready(function () {
            $("#Grid_TipoCompetencia").kendoGrid({
               ...
               , excel: {
                   fileName: "TipoCompetencia.xlsx" //nombre del archivo a exportar   
              ...
              //  , toolbar: kendo.template($scope.templateGrid_TipoCompetencia())
                , dataSource: {
                   ...
                   , schema: {
                       model: {
                           fields: {
                               ...
                               , DS_TIPO_COMPETENCIA: { type: "string" }
                               , NB_TIPO_COMPETENCIA: { type: "string" }
                           }
                       }
                   }
                }
               ...
               , columns: [{
                   headerAttributes: { style: "text-align: center; " }
                       , field: "CL_TIPO_COMPETENCIA"
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: S_TIPO_COMPETENCIA.CL_TIPO_COMPETENCIA // titulo de la columna 
                   ...
                   headerAttributes: { style: "text-align: center; " }
                       , field: "DS_TIPO_COMPETENCIA"
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: S_TIPO_COMPETENCIA.DS_TIPO_COMPETENCIA // titulo de la columna 
                       , width: 99 // ancho de la columna 
                       , hidden: false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: {
                   ...
                   headerAttributes: { style: "text-align: center; " }
                       ...
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: S_TIPO_COMPETENCIA.NB_TIPO_COMPETENCIA // titulo de la columna 
                       , width: 99 // ancho de la columna 
                       , hidden: false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: {
                   ...
                   headerAttributes: { style: "text-align: center; " }
                       ...
                       , align: "center"  //alineacion del titulo de la columna 
                   //, attributes: { title: "hola tooltip" }  
                       , title: S_TIPO_COMPETENCIA.FG_ACTIVO // titulo de la columna 
                       , width: 99 // ancho de la columna 
                       , hidden: false // indica si se esconde o no la columna 
                       , locked: false // indica si estara congelada la columna 
                       , filterable: {
               ...
               , change: function (e) {  //evento que indica cuando cambia de elemento entre una y otra linea  
                   ...
                   $scope.entidad_TipoCompetencia = dataItem; //se asigna los datos a la variable de que se crea en el scope  
               }
            });
        });
    };
    ...
    $scope.Add_Grid_TipoCompetencia = function () {
        var Obj_TipoCompetencia = { //se crea la entidad para enviarla al sp 
            CL_TIPO_COMPETENCIA: undefined
            , DS_TIPO_COMPETENCIA: ''
            , NB_TIPO_COMPETENCIA: ''
        };
        var modalInstance = $modal.open({ //se manda abrir la ventana modal para ingresar el registro 
            ...
            templateUrl: 'op_TipoCompetencia.html', //se le indica el html que se abrira 
            controller: 'Modal_TipoCompetencia', //se le indica el controlador del html 
            resolve: { //se le envia el objeto 
                OBJETO: function () { return Obj_TipoCompetencia; }
            }
        });
    };
    ...
    $scope.Edit_Grid_TipoCompetencia = function () {
        if ($scope.entidad_TipoCompetencia == undefined) { // se verifica que el registro este seleccionado  
            BootstrapDialog.show({ //se muestra el mensaje de error  
                title: S_TIPO_COMPETENCIA.TITULO_PANTALLA, //se trae el nombre de la pantalla  
        ...
        var datos = $scope.entidad_TipoCompetencia; //se asigna el registro a la variable de datos  
        var Obj_TipoCompetencia = { //se define el objeto que se enviara  
            ...
            , DS_TIPO_COMPETENCIA: datos.DS_TIPO_COMPETENCIA
            , NB_TIPO_COMPETENCIA: datos.NB_TIPO_COMPETENCIA
    ...
    $scope.Del_Grid_TipoCompetencia = function () {
        if ($scope.entidad_TipoCompetencia == undefined) { //se verifica si la entidad que guarda el evento onchange del grid no se encuentra vacia  
            BootstrapDialog.show({ //en caso de que no se encuentre se mostrara un error  
               // title: S_TIPO_COMPETENCIA.TITULO_PANTALLA, //se le pone el titulo especificado en la clase   
            ...
            buttons: [{ //se le agregan botones  
                ...
                action: function (result) { //se especifica la funcion que hara este boton  
                    if (result) { //en caso de que sea afirmativo  
                        $scope.Eliminar_TipoCompetencia();  //se ejecuta la funcion de elimina_?? especificada para borrar el registro  
                    }
                }
            }, {
    ...
    $scope.Eliminar_TipoCompetencia = function () {
        var datos = $scope.entidad_TipoCompetencia;  //se extrae el registro obtenido en el evento change del grid 
        $http({ //se especifica cual sera nuestro web service para ejecutarlo 
            url: "ws/OperacionesGral.svc/Delete_S_TIPO_COMPETENCIA",
            method: "POST",
            data: { //se le pasan los parametros 
                CL_TIPO_COMPETENCIA: datos.CL_TIPO_COMPETENCIA, // se le pasa la llave primaria  
                usuario: TraerUsuario(), // se le pasa el usuario 
                //ojo modificar   
                programa: "CatalogoTipoCompetencia.html"  //se le pasa  
            },
        ...
        .then(function (response) { // ejecucion de WS 
            if (response.data != true) {  //si el procedimiento regresa falso 
                BootstrapDialog.show({ //se mostrara un mensaje de error  
                    title: S_TIPO_COMPETENCIA.TITULO_PANTALLA,  //el titulo del mensaje esta en la clase Config que esta instanciada en appPrincipal.js en el atributo MENSAJEERROR  
                ...
                $("#Grid_TipoCompetencia").data("kendoGrid").dataSource.read(); //se manda a refrescar el grid  
                $("#modalDelete").modal('toggle');
                BootstrapDialog.show({  //si el procedimiento regresa true 
                    title: S_TIPO_COMPETENCIA.TITULO_PANTALLA,  //el titulo del mensaje  
        ...
        function (response) {  //funcion que mostrara el mensaje si ocurre un error antes de ejecutar el sp  
            BootstrapDialog.show({
                title: S_TIPO_COMPETENCIA.TITULO_PANTALLA,
    ...
    $scope.CargarGrid_TipoCompetencia();


    
    $scope.ActivarDesactivar_Grid_TipoCompetencia = function () {
        var datos = $scope.entidad_TipoCompetencia;   //se extrae el registro obtenido en el evento change del grid 
        var tipoOperacion = "A"
        var Obj_TipoCompetencia = {// se declara el objeto con la estructura del result para insertar o actualizar el registro 
            CL_TIPO_COMPETENCIA: datos.CL_TIPO_COMPETENCIA
 			, DS_TIPO_COMPETENCIA: datos.DS_TIPO_COMPETENCIA
 			, NB_TIPO_COMPETENCIA: datos.NB_TIPO_COMPETENCIA
            , FG_ACTIVO: !datos.FG_ACTIVO
        ...
        $http({
            url: "ws/OperacionesGral.svc/Insert_update_S_TIPO_COMPETENCIA",
            method: "POST",
            data: {
                V_S_TIPO_COMPETENCIA: Obj_TipoCompetencia,
                usuario: TraerUsuario(),
                //ojo modificar   
                programa: "InsertaTipoCompetencia.html",
...
//MODAL QUE ABRE LA VENTANA DE INSERTAR/EDITAR S_TIPO_COMPETENCIA 
app.controller('Modal_TipoCompetencia', function ($scope, $modalInstance, $http, OBJETO) {
    $scope.CL_TIPO_COMPETENCIA = OBJETO.CL_TIPO_COMPETENCIA;
    $scope.DS_TIPO_COMPETENCIA = OBJETO.DS_TIPO_COMPETENCIA;
    $scope.NB_TIPO_COMPETENCIA = OBJETO.NB_TIPO_COMPETENCIA;
    ...
    $scope.lbl_CL_TIPO_COMPETENCIA = S_TIPO_COMPETENCIA.CL_TIPO_COMPETENCIA;
    $scope.ph_CL_TIPO_COMPETENCIA = S_TIPO_COMPETENCIA.CL_TIPO_COMPETENCIA_ph;
    $scope.lbl_DS_TIPO_COMPETENCIA = S_TIPO_COMPETENCIA.DS_TIPO_COMPETENCIA;
    $scope.ph_DS_TIPO_COMPETENCIA = S_TIPO_COMPETENCIA.DS_TIPO_COMPETENCIA_ph;
    $scope.lbl_NB_TIPO_COMPETENCIA = S_TIPO_COMPETENCIA.NB_TIPO_COMPETENCIA;
    $scope.ph_NB_TIPO_COMPETENCIA = S_TIPO_COMPETENCIA.NB_TIPO_COMPETENCIA_ph;
    $scope.lbl_FG_ACTIVO = S_TIPO_COMPETENCIA.FG_ACTIVO;
    $scope.ph_FG_ACTIVO = S_TIPO_COMPETENCIA.FG_ACTIVO_ph;
    ...
    if ($scope.CL_TIPO_COMPETENCIA != undefined)
        $scope.CL_TIPO_COMPETENCIA_disable = true;
      //$document.find('txt_CL_TIPO_COMPETENCIA').disabled = true;
    //    document.getElementById("txt_CL_TIPO_COMPETENCIA").disabled = true;
    ...
    $scope.ok = function () { //en caso de que le den click en aceptar generara lo siguiente  
        ...
        if ($scope.CL_TIPO_COMPETENCIA != undefined)  //se verifica que la clave primaria sea diferente de nulo  
        ...
        var Obj_TipoCompetencia = {// se declara el objeto con la estructura del result para insertar o actualizar el registro 
            CL_TIPO_COMPETENCIA: $scope.CL_TIPO_COMPETENCIA
 			, DS_TIPO_COMPETENCIA: $scope.DS_TIPO_COMPETENCIA
 			, NB_TIPO_COMPETENCIA: $scope.NB_TIPO_COMPETENCIA
    ...
    $scope.cancel = function () {
        $("#Grid_TipoCompetencia").data("kendoGrid").dataSource.read();
